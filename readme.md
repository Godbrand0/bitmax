...
**Landing page** (`components/Landing.tsx`, shown as the logged-out state on `/` rather than a separate route): hero, a trust strip ("Non-custodial," "Bitcoin-backed (sBTC)," "Withdraw anytime"), four detailed how-it-works cards, a dedicated "boosting is what BitMax is about" section (echoing the section 2.3/6a framing — boosting isn't a side feature), a "borrow without leaving" section explaining the embedded Zest integration, an FAQ (plain `<details>`/`<summary>`, no JS state needed), and a final CTA. Deliberately makes no unverifiable claims — no APY numbers, no "audited" claim — since none of that is true yet.
