...
**Update: `mock-ststxbtc-pool.clar` is gone — `ve-stx-lock.clar` now calls StackingDAO's real `stacking-dao-core-ststxbtc-v2` and `ststxbtc-tracking-v2` directly**, mirroring `bitmax-vault.clar`'s real stBTC integration exactly (trait-typed params, pinned to the real mainnet principal, mainnet-only by construction). `bitmax-boost-distributor.clar`'s `close-epoch` now takes the real sBTC token and `ststxbtc-tracking-v2` as trait-typed params too, and forwards them into a new `ve-stx-lock.claim-boost-reward` (owner-gated to the registered distributor) that does the actual external claim and relays the sBTC out. `close-epoch` computes `total-weight` first and skips the claim entirely when nobody has an active lock, so nothing gets stranded in StackingDAO with no locker to pay. Local test coverage dropped accordingly (23 tests now, down from 29) for the same reason `bitmax-vault.clar`'s real leg has no local coverage — simnet can't reach StackingDAO's real contracts. One live operational note: as of this integration, StackingDAO has deposits, init-withdraw, *and* withdraw all administratively shut down on `stacking-dao-core-ststxbtc-v2` (confirmed via live reads) — almost certainly a maintenance/migration pause, but it means this leg can't be exercised end-to-end on mainnet until they re-enable it, regardless of how correct the integration is.
